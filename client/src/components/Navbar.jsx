...
const Navbar = ({ setShowLogin }) => {
  const location = useLocation(); // Gets the current URL location object (pathname, search params, etc.) using React Router
  const [open, setOpen] = useState(false); // Manages the open/closed state of something (like a dropdown or sidebar); initially closed
  const navigate = useNavigate(); // Hook from React Router to programmatically navigate between pages

  return (
    <div
      className={`flex items-center justify-between px-6 md:px-16 lg:px-24
    x;:px-32 py-4 text-gray-600 border-b border-borderColor relative transition-all
    ${location.pathname === "/" && "bg-light"}`}
    >
      <Link to="/">
        <img src={assets.logo} alt="logo" className="h-8" />{" "}
        {/* "alt" describes the image as "logo" for accessibility */}
      </Link>
